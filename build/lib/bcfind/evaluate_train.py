...
def localize_and_evaluate(
    ...
    f_name = os.path.basename(y_file).split(".")[0]
    y = pd.read_csv(open(y_file, "r"))[["#x", " y", " z"]]

    if exclude_border is not None:
        data_shape = x.shape
        y = bcf_ut.remove_border_points_from_df(
            y, ["#x", " y", " z"], data_shape, exclude_border
        )
    ...
    if outdir is not None:
        ...
        evaluation.to_csv(f"{outdir}/Pred_centers/pred_{f_name}.marker", index=False)

    TP = np.sum(evaluation.name == "TP")
    FP = np.sum(evaluation.name == "FP")
    FN = np.sum(evaluation.name == "FN")
...
def main():
    ...
    unet = build_unet(
        ...
        opts.exp.exclude_border,
    )
    unet.load_weights(f"{opts.exp.unet_checkpoint_dir}/model.h5")

    # Build DoG with trained parameters
    dog = BlobDoG(
        len(opts.data.data_shape),
        opts.data.dim_resolution,
        np.array(opts.exp.exclude_border) * 2,
    )
    try:
    ...
    preprocessing_fun = ft.partial(
        ...
        downscale_factors=opts.preproc.downscale,
        pad_output_shape=opts.data.data_shape,
    )
    scaler = Scaler(opts.preproc.normalization, opts.preproc.standardization)
    ...
    data = (
        tf.data.Dataset.from_tensor_slices(X_train[..., np.newaxis])
        .batch(BATCH_SIZE)
        .prefetch(PREFETCH)
    )
    ...
    for batch in data:
        ...
        pred_batch = bcf_ut.sigmoid(pred_batch) * 255
    ...
    data = (
        tf.data.Dataset.from_tensor_slices(X_test[..., np.newaxis])
        .batch(BATCH_SIZE)
        .prefetch(PREFETCH)
    )
    del X_test
