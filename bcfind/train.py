...
def parse_args():
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="Test set will be used as validation during training. No early stopping will be however applied",
    )
...
def main():
    ...
    train_tiff_files = sorted(
        [f"{conf.data.train_tif_dir}/{fname}" for fname in train_fnames]
    )
    train_marker_files = sorted(
        [f"{conf.data.train_gt_dir}/{fname}.marker" for fname in train_fnames]
    )

    test_fnames = os.listdir(conf.data.test_tif_dir)
    test_tiff_files = sorted(
        [f"{conf.data.test_tif_dir}/{fname}" for fname in test_fnames]
    )
    test_marker_files = sorted(
        [f"{conf.data.test_gt_dir}/{fname}.marker" for fname in test_fnames]
    )
    ...
    if not args.only_dog:
        ...
        if conf.unet.model == "unet":
            ...
            raise ValueError(
                f'UNet model must be one of ["unet", "se-unet", "eca-unet", "attention-unet", "moe-unet"]. \
        ...
        loss = FramedCrossentropy3D(
            conf.unet.exclude_border, conf.unet.input_shape, from_logits=True
        )

        prec = Precision(
            0.036, conf.unet.input_shape, conf.unet.exclude_border, from_logits=True
        )
        rec = Recall(
            0.036, conf.unet.input_shape, conf.unet.exclude_border, from_logits=True
        )
