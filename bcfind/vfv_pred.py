...
def put_substack_in_q(
    ...
    if np.mean(substack) < min_thresh:
        logging.debug(f"Substack mean = {np.mean(substack)} < {min_thresh}. Skipping ")
        return
    ...
    logging.debug("Putting in queue...")
    queue.put([substack, sub_name])
    n = queue.qsize()
    logging.debug(f"Substack_queue has {n} elements.")
    return
...
def find_cells(img, localizer, outfile=None):
    ...
    if outfile is not None:
        logging.debug(f"Saving coordinates to {outfile}")
...
def make_cloud(centers_dir, scale=None):
    ...
    for f_name in f_names:
        try:
            ...
            logging.info(f"DEBUG: error loading file {f_name}")
            raise Exception(e)
...
def predict_vfv(
    ...
    def nn_worker():
        while True:
            ...
            logging.debug(f"UNet prediction on substack {name}")
            nn_pred = nn_model(
            ...
            n = embedding_q.qsize()
            logging.debug(f"Embedding_queue has {n} elements")
            substack_q.task_done()
    ...
    def localizer_worker():
        while True:
            ...
            logging.debug(f"DoG prediction on substack {name}")
            cells = find_cells(nn_pred, localizer, outfile)
            embedding_q.task_done()
            if cells.shape[0] > 0:
                logging.debug(f"{cells.shape[0]} cells found.")
...
def parse_args():
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="""Set verbosity to True. If False (default) only load and save processes are printed. 
        If True all substack processes will be printed.""",
    )
...
def main():
    ...
    if os.path.isfile(dog_par_file):
        ...
        logging.info("No optimal DoG parameteres found, assigning defaults.\n")
    ...
    logging.info(f"VirtualFusedVolume with shape = {vfv.shape}")
    logging.info(f"{n} patches to complete the whole prediction.\n")
