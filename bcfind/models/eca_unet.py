...
from bcfind.layers.efficient_channel_attention import EfficientChannelAttention, _get_eca_kernel_size
...
class ECAUNet(tf.keras.Model):
    def __init__(self, n_blocks, n_filters, k_size, k_stride, dropout=None, regularizer=None, **kwargs):
        ...
        for i in range(self.n_blocks):
            ...
            eca = EfficientChannelAttention(_get_eca_kernel_size(self.n_filters * (2**i)))
        ...
        for i in range(self.n_blocks):
            ...
            if i < self.n_blocks - 1: # last block has no ECA module
                eca = EfficientChannelAttention(_get_eca_kernel_size(self.n_filters * (2 ** (self.n_blocks - i - 2)) * 2))
