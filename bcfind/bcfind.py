...
from bcfind.utils import preprocessing
from bcfind.make_training_data import make_data
from bcfind.train import build_unet, unet_fit, dog_fit
from bcfind.bipartite_match import bipartite_match
from bcfind.evaluate_train import predict_file
...
class BCFind:
    ...
    @staticmethod
    def predict(unet, dog, img, save_res=False, outfile=None):
        ...
        :param img: (numpy.ndarray) the image on which making the prediction.
        :param save_res: (bool) if the prediction results must be saved or not. Default to False.
        :param outfile: (string or path) path to the file for saving the prediction results.
        :returns: a numpy.ndarray of coordinates.

        """
        centers = predict_file(unet, dog, img, save_res, outfile)
        return centers

    @staticmethod
    def evaluate(pred, true, max_dist, dim_resolution=[1.0, 1.0, 1.0]):
        """Method for evaluating the predictions with respect to the true locations. It return the counts of true positives,
        ...
        :returns: three integers represting the numbers of true positives, false positives and false negatives.

        """
        _, _, TP, FP, FN = bipartite_match(true, pred, max_dist, dim_resolution)
        return TP, FP, FN
