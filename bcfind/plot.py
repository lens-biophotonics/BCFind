...
def get_idxs_in_section(array, section_center, section_width=1, axis=0):
    in_section = (array[:, axis] >= section_center - section_width // 2) & (array[:, axis] < section_center + section_width // 2 + 1)
    return in_section


def make_video(input, target=None, nn_pred=None, centers=None, center_persistance=5, slide_axis=0, out_filename=None, fps=2):
    ...
    if centers is not None:
        ...
        in_section = get_idxs_in_section(centers, section_center=0, section_width=center_persistance, axis=slide_axis)
    ...
    def animate_func(z):
        ...
        if centers is not None:
            in_section = get_idxs_in_section(centers, section_center=z, section_width=center_persistance, axis=slide_axis)
            
            sct.set_offsets(np.c_[centers[in_section, plt_axes[1]], centers[in_section, plt_axes[0]]])
            sct.set_edgecolors(c=colors[in_section])
            
            if nn_pred is not None:
                sct2.set_offsets(np.c_[nn_pred_centers[in_section, plt_axes[1]], nn_pred_centers[in_section, plt_axes[0]]])
