...
class SqueezeAndExcite(tf.keras.layers.Layer):
    def __init__(self, n_input_channels, squeeze_factor=2, **kwargs):
        ...
        self.squeeze = tf.keras.layers.Conv3D(
            self.n_input_channels // self.squeeze_factor, 1, 1, activation="relu"
        )
        self.excite = tf.keras.layers.Conv3D(
            self.n_input_channels, 1, 1, activation="sigmoid"
        )
