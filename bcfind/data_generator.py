...
class BatchGenerator(tf.keras.utils.Sequence):
    ...
    def add_random_gamma(self, gamma_range, p=0.5):
        self.operations['gamma'] = ft.partial(bcf_aug.random_gamma, gamma_range=gamma_range, p=p)

    def add_random_contrast(self, alpha_range, p=0.5):
        self.operations['contrast'] = ft.partial(bcf_aug.random_contrast, alpha_range=alpha_range, p=p)
    
    def add_random_brightness(self, alpha_range, p=0.5):
        self.operations['brightness'] = ft.partial(bcf_aug.random_brightness, alpha_range=alpha_range, p=p)

    def add_random_zoom(self, alpha_range, p=0.5):
        self.operations['zoom'] = ft.partial(bcf_aug.random_zoom, alpha_range=alpha_range, p=p)
    
    def add_random_gauss_filter(self, sigma_range, p=0.5):
        self.operations['gauss_filter'] = ft.partial(bcf_aug.random_gauss_filter, sigma_range=sigma_range, p=p)
    
    def add_random_noise(self, sigma_range, p=0.5):
        self.operations['noise'] = ft.partial(bcf_aug.random_noise, sigma_range=sigma_range, p=p)
    ...
    def _augment_data(self, x_batch, y_batch):
        for op in self.operations:
            if op == 'zoom':
                x_batch, y_batch = self.operations[op](x_batch, y_batch)
            else:
                x_batch = self.operations[op](x_batch)
